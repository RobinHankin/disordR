...
For example, consider a set `S` of real numbers; the "first" value of
`S` is implementation specific (because a set does not specify the
order of its elements)...but the maximum value of `S` has a
well-defined result.  The `disordR` package makes forbidden operations
of this type impossible while allowing transparent R idiom for
permitted operations.  Note that one cannot dispense with order
