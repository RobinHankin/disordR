...
Experimental class `disindex` is a means by which extraction methods
operate with the output of `which()`.  Consider the following R
session:
...
Observe that there is no sense in which the `wanted` element of `dl`
is the "second":
